...
console.log(color('  >>>>>>>>>>>>>>>??????>>||||||||||||'))
    ...
    simple.on('open', () => {
        success('2', 'Done Udah Nyambung  Subscribe snapdxkz' )
    })
...
simple.sendMessage(anu.jid, "_*Makasih Udah Add Saia Bangh, Halo Abangh² Aku SnapBotzz Silahkan ketik .menu Untuk Memulai*_")
}
...
teks = `Halo Bangh @${anu_user}\nSelamat Datang Di Group: ${mdata.subject}\n\n*INFO USER*\n🍧 _*NAMA : @${anu_user}*_\n🍧 _*NOMER : @${num.split('@')[0]}*_\n🍧 _*UMUR : UDAH MAU MATI*_\n🍧 _*KELAS : DAH PUNYA ANAK*_\n\n_*Semoga Hari² Mu Menyenangkan*_`
sendButImage(anu.jid, teks, `© SnappyXD`, img,but = [{buttonId:`hallo`, 
buttonText:{displayText: 'Welcome Bangh'},type:1}], options = {contextInfo: {mentionedJid: [num, number]},thumbnail: Buffer.alloc(0)})
...
img = await getBuffer(ppUrl)
teks = `Bye,Gausah Balik Lagi Kntl @${num.split('@')[0]}\nSemoga Amal IBdahnya Si Terima Di sisi Tuhan Yang Maha Esa Selamat Tinggal Di Group: ${mdata.subject}`
sendButImage(anu.jid, teks, `© SnappyXD`, img,but= [{buttonId: `byebye`, buttonText: {displayText: `See You Broo?👋!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, number]}, thumbnail: Buffer.alloc(0)})
